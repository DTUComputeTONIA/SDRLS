...
function mergeexperiments(path::String)
    ...
    mainexperiment = loadexperiment(jld2files[1])
    for ff in jld2files
        exp = loadexperiment(ff)
        for jobindex = 1:length(mainexperiment.jobs)
            job = mainexperiment.jobs[jobindex]
            if job.status != finished
                ...
                if !isnothing(jj)
                    mainexperiment.jobs[jobindex] = exp.jobs[jj]
                end
            end
        end
    end
    date = Dates.format(now(), "mmddHHMMSSs")
    saveexperiment(path * "/merged_data-$(date).jld2", mainexperiment)
    mainexperiment
end
...
function mkworkspace(exp::Experiment)
    ...
    cp("src/experiments/_compile.jl", workspace * "/compile.jl")
    cp("src/experiments/_fetch.jl", workspace * "/fetch.jl")
