...
function createworkspaceinserver(exp::Experiment, hpc::HPC)
    ...
    sshcommand(hpc, "mkdir -p /work3/amraj/$workspace")
    run(`scp -r src $(hpc.user)"@"$(hpc.server)":/work3/amraj/"$workspace`)
    run(`scp -r $workspace"/." $(hpc.user)"@"$(hpc.server)":/work3/amraj/"$workspace`)
...
function submitjob(exp::Experiment, hpc::HPC)
    ...
    workspace = exp.workspace
    bash_file = "job.sh"
    thecompile = "cd /work3/amraj/$workspace && julia /work3/amraj/$workspace/compile.jl"
    @info sshcommand(hpc, thecompile)
    thejob = "cd /work3/amraj/$workspace && for i in {1..$tasknumber};  do bsub < $bash_file -env SERVER_ID=\$i ; sleep 1; done"
...
function fetchresults(exp::Experiment, hpc::HPC)
    workspace = exp.workspace
    hpcresult = workspace * "/hpc-" * hpc.server * "_" * Dates.format(now(), "mmddHHMMSS")
    if !ispath(hpcresult)
        mkdir(hpcresult)
    end
    thecompile = "cd /work3/amraj/$workspace && julia /work3/amraj/$workspace/fetch.jl"
    @info sshcommand(hpc, thecompile)
    run(`scp -r $(hpc.user)"@"$(hpc.server)":/work3/amraj/"$workspace"/results/merged_data*" $hpcresult`)
